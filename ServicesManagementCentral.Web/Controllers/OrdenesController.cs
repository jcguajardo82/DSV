...
namespace ServicesManagement.Web.Controllers
{
    ...
    , string bolsas, string contenedores, string hieleras, string checkPinPad)
        ...
        public ActionResult BuscarCotizacionesLogytMonitor(List<ProductEmbalaje> Products, int orderNo)
        {
            try
            {
                string productsAll = string.Empty;
                ...
                DataSet ds = DALProcesoSurtido.upCorpOms_Cns_MonitorCobertura(orderNo, sumPeso, bigTicket);
                bool TieneCobertura = false;
                string msg = "No tiene cobertura";
                if (ds.Tables[0].Rows[0]["mensaje"].ToString() == "SI")
                {
                    TieneCobertura = true;
                    msg = "Tiene cobertura";
                }
                var result = new { Success = true, resp = TieneCobertura, Message = msg };
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            catch (Exception x)
            {
